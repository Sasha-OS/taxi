...
class DatabaseSingleton {
    ...
    createTable(table) {
        // if (document.getElementById('input1').value == password ) {
            this.from = 0;
...
function deleteElement(table, id) {
    console.log(id)
    let str = DB.URL.concat("delete from " + table + " where driverid = " + id.substring(1));
    console.log(str)
...
const DB = new DatabaseSingleton()
   ...
   function showCover() {
    ...
    document.getElementById('myBtn').onclick = function() {
      showPrompt("Password", function(value) {
        if (value == password) {
